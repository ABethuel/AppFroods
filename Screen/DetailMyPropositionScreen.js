...
class DetailMyPropositionScreen extends React.Component {
  ...
  _displayAddRemoveBtn() {
    if (this.props.propositionCommand.findIndex(item => item.id === this.state.dish.id) !== -1){
      return(
        <View>
          <Text style={styles.text_add}>Annuler la proposition</Text>
        </View>
      )
    }
    else {
      return(
        <Text style={styles.text_add}>Proposer à nouveau</Text>
      )
    }
  }

  _toggleCommand() {
    const action = { type: 'TOGGLE_PROPOSITION', value: this.state.dish}
    ...
    return(
      <View style={styles.main_container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.text}>Détail de la proposition</Text>
        </View>
...
export default connect(mapStateToProps)(DetailMyPropositionScreen)